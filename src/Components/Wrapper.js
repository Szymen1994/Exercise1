...
class Wrapper {
  ...
  getUserGist(user, pageNumber, resultPerPage) {
    return this.getRequest(
      `/users/${user}/gists?page=` + pageNumber + "&per_page=" + resultPerPage
    );
  }
  //?per_page=40&page=1
  ...
  publicGist(pageNumber, resultPerPage) {
    return this.getRequest(
      "/gists/public?page=" + pageNumber + "&per_page=" + resultPerPage
    );
  }
}

export let token = "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";
